  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          <h1 className="text-3xl font-extrabold text-gray-900 tracking-tight">{t('attendanceReports')}</h1>
          <p className="text-gray-500 mt-1 text-sm">{language === 'ar' ? 'قم بتصفية واستخراج تقارير الحضور لموظفيك' : 'Filter and extract attendance reports for your employees'}</p>
        </div>
      ...
      <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 mb-8">
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4 items-start">
          <div className="lg:col-span-2">
            ...
            <ClientSelect value={clientId} onChange={setClientId} hideLabel noMargin />
          </div>
          <div>
              ...
              <tr className="bg-gray-50/80 border-b border-gray-100 text-gray-500 text-sm font-semibold">
                ...
                <th className="p-4 border-r border-gray-100 text-center w-24">
                  <div className="flex flex-col items-center justify-center gap-2">
                    <label className="text-[10px] flex items-center gap-1.5 cursor-pointer mb-0 hover:text-gray-700 transition-colors" title={language === 'ar' ? 'تحديد الكل' : 'Select all'}>
                    ...
                    <label className="text-[10px] flex items-center gap-1.5 cursor-pointer text-emerald-600 hover:text-emerald-700 transition-colors mb-0" title={language === 'ar' ? 'تحديد هذه الصفحة فقط' : 'Select this page only'}>
            ...
            <tbody>
              {paginatedEmployees.length > 0 ? (
                paginatedEmployees.map((emp, index) => (
                  <tr key={emp.id || emp._id} className="border-b border-gray-50 hover:bg-emerald-50/30 transition-colors group">
