  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          <h1 className="text-3xl font-extrabold text-gray-900 tracking-tight">{t('employeePerformance')}</h1>
          <p className="text-gray-500 mt-1 text-sm">{language === 'ar' ? 'تتبع وقيّم أداء موظفيك بشكل شهري ودقيق' : 'Track and evaluate employee performance monthly'}</p>
        </div>
        <div className="flex flex-wrap gap-3">
          ...
          <button onClick={handlePrintPerformance} className="flex items-center gap-2 bg-white border border-gray-200 text-gray-700 px-6 py-2.5 rounded-xl font-medium hover:bg-gray-50 hover:border-gray-300 transition-all active:scale-95">
            ...
            <tbody>
              {paginatedEmployees.length > 0 ? paginatedEmployees.map((emp, index) => (
                <tr key={emp.id || emp._id} className="border-b border-gray-50 hover:bg-emerald-50/30 transition-colors">
                  ...
                  <td className="p-4 border-r border-gray-100 text-center"><input type="checkbox" checked={!!selectedEmpIds[emp.id || emp._id]} onChange={() => handleCheckboxChange(emp.id || emp._id)} className="rounded text-emerald-600 focus:ring-emerald-500 w-4 h-4" /></td>
